...
import { Remove } from "../../redux/features/wishList/wishSlice";
    ...
    <div className="px-4">
          ...
          {wishListItems.map((item) => (
            <tr className=" border-b-2" key={item.id}>
              <td className="flex flex-col md:flex-row justify-start items-start md:items-center p-4 text-base sm:text-lg text-gray-600 font-semibold">
                <img src={item.image} className=" w-20 h-auto mx-4" alt="product img"/>
                {item.title}
              </td>
      ...
      <div className="flex flex-row justify-center items-center">
        {wishListItems.length === 0 ? (
          <TbHeartPlus className="text-9xl text-slate-800 mb-12" />
        ) : null}
      </div>
    </div>
  );
